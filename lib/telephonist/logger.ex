defmodule Telephonist.Logger do
  ...
  def on_event(:lookup_success, {sid, status, state}) do
    log sid, "Call found in status #{inspect status} and state #{inspect state}"
  end
  ...
  def on_event(:attempt_transition, {sid, state_machine, state_name, twilio, options}) do
    log sid, "Calling #{state_machine}.transition(#{inspect state_name}, #{inspect twilio}, #{inspect options})"
  end

  def on_event(:transition_error, {sid, exception, state_machine, state_name, twilio, options}) do
    log sid, "Transition failed! Calling #{state_machine}.on_transition_error(#{inspect exception}, #{inspect state_name}, #{inspect twilio}, #{inspect options})"
  end
